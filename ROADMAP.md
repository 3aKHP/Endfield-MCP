...
| 版本 | 主题 | 内容 |
|------|------|------|
| **0.3.1** | 技术债清理 | (1) ✅ npm Trusted Publishing 迁移（PR #7，dev）——npm 侧配 trusted publisher，cd.yml 加回 `--provenance`，去掉 NPM_TOKEN；(2) ✅ S7: `ef_search_characters` 加 `.max(200)` ReDoS 防护（PR #5）；(3) ✅ Story bundled data（PR #6）——19MB story bundle 进 npm 包，离线兜底；(4) ⏳ Mirror CI workflow——本仓库消费侧契约补齐（PR #8），EndFieldGameData 仓库的自动重导出 GitHub Actions 待 self-hosted runner |
