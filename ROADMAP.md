...
| **0.3.2** | 体验优化 + 代码债清理 | (1) ✅ R1: `{id, text}` 本地化类型统一——删除 `characterProfiles.ts` 的 `RecordField` + `characterTable.ts` 的 `LocalizedField` / `CvField` 三个私有重复定义，全部改用 `texts.ts` 的 `LocalizedText`，消除 4 处 `as` 强转（PR #13）；(2) 🚫 R2 已评估放弃：`characterEnums.ts` 枚举动态化 wontfix——枚举值是已验证常量且极少变，动态化唯一实质收益（多语言职业名）当前用不到（YAGNI），且 weaponType 无源表会留下混合形态破坏单一职责、纯数据模块引入 store 依赖违反分层初衷，成本不抵收益。若未来角色工具需 `lang` 参数返回本地化职业名再重评估；(3) ⏳ 体验优化：工具描述关键词优化（提升客户端 RAG 召回）/ 分页标准化 `{total, offset, limit, items}` / 结构化错误 `{error_code, message}` |
