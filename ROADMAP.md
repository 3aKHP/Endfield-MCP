...
- TypeScript: `0.4.0-dev.0` — in development (v0.4 target: Worldbuilding — in-game PRTS documents & wiki entries). Latest release: `0.3.4` (hotfix — release sync now lists releases and matches by asset name instead of assuming `/releases/latest` carries every asset; the v0.4.0 worldview Release made tables/story syncs report no_data).
...
| **0.3.4** | hotfix | ✅ sync 多资产兼容修复——`checkLatestRelease` 从 `/releases/latest`（返回整个仓库最新 Release，假设其含所有资产）改为 `/releases` 列表按 `assetName` 匹配。根因：v0.4.0 镜像发独立 worldview 资产后，tables/story sync 去最新 Release（v0.4.0）找各自资产失败 → no_data。开发 v0.4 live smoke 时发现 |
