...
| **0.3.2** | 体验优化 + 代码债清理 | (1) ✅ R1: `{id, text}` 本地化类型统一——删除 `characterProfiles.ts` 的 `RecordField` + `characterTable.ts` 的 `LocalizedField` / `CvField` 三个私有重复定义，全部改用 `texts.ts` 的 `LocalizedText`，消除 4 处 `as` 强转（refactor 分支）；(2) ⏳ R2: `characterEnums.ts` 枚举动态化——profession/charType 从 `CharProfessionTable`/`CharTypeTable` 动态读取，weaponType 无源表保持硬编码（设计抉择待定）；(3) ⏳ 体验优化：工具描述关键词优化（提升客户端 RAG 召回）/ 分页标准化 `{total, offset, limit, items}` / 结构化错误 `{error_code, message}` |
